...
function 闪一下(obj) {
    //0.5秒耗时
    obj.changeOpacity(0.1, { t: ">", duration: 0.3 * fast, ease: "power2.in" })
    obj.changeOpacity(0.9, { t: ">", duration: 0.5 * fast, ease: "power2.Out" })
}
...
for (let key in wordCfg.data) {
    ...
    //----男老师in
    teacher.m.moveTo({ t: ">", y: -0.4, x: -4, duration: teacherMoveTime * fast, ease: "expo.out" })
    //----朗读
    ...
    if (key % 2) {
        console.log('左边')
        移到一边(obj.en, startx, starty) //0.8秒耗时
        移到一边(obj.zh, startx, starty - 0.3)//0.8秒耗时
        if (key != 0) { starty = starty - 0.8 }
    } else {
        console.log('右边')
        移到一边(obj.en, startx * -1 + 0.2, starty) //0.8秒耗时
        移到一边(obj.zh, startx * -1 + 0.2, starty - 0.3) //0.8秒耗时
    }
}
...
if (movieSlice['排列'] == 1) {
    ...
    for (let key in objLs) {
        ...
        obj.zh4m = wordCfg.data[key].zh4m
        //---en朗读一次 2.3秒
        objLs[key].en.changeOpacity(1, { duration: 0.8 * fast, ease: "none" })
        ...
        objLs[key].zh.changeOpacity(1, { duration: 0.8 * fast, ease: "none" })//取消文字半透明
