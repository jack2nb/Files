...
for (let key in wordCfg.data) {
    ...
    //----创建单词
    obj.zh = obj.g.addText(obj.zhWord, { position: [0.2, 0.6], opacity: 0.9, scale: 0.65, color: "#0a0a0a" })
    obj.en = obj.g.addText(obj.enWord, { position: [0.2, -0.6], opacity: 0.9, scale: 0.65, color: "#0a0a0a" })
    //---老师in
    teacher.f.show({ duration:  0.001}).moveTo({ y: -0.4, x: -4.9, duration: 1.8*fast, ease: "expo.out" })
    //----单词朗读
    obj.zh.flyIn({ duration: 1.1*fast });
    闪一下(obj.zh)
    进度线(1*fast, 0.2, 0.07)
    obj.en.wipeIn({ duration: 1.1*fast });
    闪一下(obj.en) //异步才能获取坐标 object3D
    进度线(1*fast, 0.2, -1.3)
    //----老师out
    teacher.f.show({ duration:  0.001}).moveTo({ y: -0.4, x: -10, duration: 1.8*fast, ease: "expo.in" })
    //----老师in
    teacher.m.show({ duration:  0.001}).moveTo({ y: -0.4, x: -4.9, duration: 1.8*fast, ease: "expo.out" })
    //----单词朗读
    ...
    闪一下(obj.en) //异步才能获取坐标 object3D
    进度线(1*fast, 0.2, -1.3)
    teacher.m.show({ duration: 0.001}).moveTo({ y: -0.4, x: -10, duration: 1.8*fast, ease: "expo.in" })
...
for (let key in objLs) {
    ...
    objLs[tmpInt].zh.moveTo({ t: "<", y: starty-1.4, x: startx, duration: 1.1*fast, ease: "expo.in" })
